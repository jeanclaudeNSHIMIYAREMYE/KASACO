...
from ...utils import (
    extract_server_url_from_procfile,
    get_app_path,
    get_package_json_path,
    get_tailwind_src_path,
    install_pip_package,
)
...
class Command(BaseCommand):
    ...
    def handle(self, *args, method, **options):
        ...
        if method != self.handle_init_command:
            ...
            self.is_standalone = get_config(
                "TAILWIND_USE_STANDALONE_BINARY"
            ) or not os.path.exists(get_package_json_path(app_name))
    ...
    def handle_init_command(self, **options):
        ...
        tailwind_version = (
            options["tailwind_version"].strip()
            if options.get("tailwind_version")
            else None
        )
        app_template_choice = {"4s": "1", "4": "2", "3": "3", None: None}[
            tailwind_version
        ]
        if not app_template_choice:
            app_template_choice = input(
                """Choose template:
    ...
    def handle_plugin_install_command(self, plugin_name, **options):
        ...
        if plugin_line in content:
            return self.print_warning(
                f"Plugin {plugin_name} is already included in styles.css"
            )
        ...
        if import_line not in content:
            return self.print_error(
                'Could not find @import "tailwindcss"; in styles.css'
            )
