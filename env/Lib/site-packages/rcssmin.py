...
def _make_cssmin(python_only=False):
    ...
    uri_space_sub = _re.compile(
        (r"(%(escape)s+)|%(spacechar)s+|%(nl_escaped)s+") % locals()
    ).sub
    uri_space_subber = lambda m: m.groups()[0] or ""

    space_sub_simple = _re.compile((r"[\r\n\f\040\t;]+|(%(comment)s+)") % locals()).sub
    space_sub_banged = _re.compile(
        (r"[\r\n\f\040\t;]+|(%(_bang_comment)s+)") % locals()
    ).sub
...
if __name__ == "__main__":
    ...
    def main():
        ...
        keep_bang_comments = (
            "-b" in _sys.argv[1:] or "-bp" in _sys.argv[1:] or "-pb" in _sys.argv[1:]
        )
        if "-p" in _sys.argv[1:] or "-bp" in _sys.argv[1:] or "-pb" in _sys.argv[1:]:
            ...
            xcssmin = cssmin
        _sys.stdout.write(
            xcssmin(_sys.stdin.read(), keep_bang_comments=keep_bang_comments)
        )
